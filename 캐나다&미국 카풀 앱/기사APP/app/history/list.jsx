...
export default function Page({ }) {
    ...
    const dataFunc = async (reset) => {
        ...
        const { data, error } = await API.post('/v2/driver/history/rideHistory');
        // console.log(JSON.stringify(data?.carpoolList?.[0], null, 2))
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <PostTicket item={parse} style={{ marginHorizontal: rootStyle.side, marginBottom: 23 }} onPress={() => {
                if(item?.joins?.length < 1) {
                    ToastMessage(lang({ id: 'the_trip_ended_with_no_passengers_on_board' }), { type: 'error' });
                    return;
                }

...
export default function Page() {
    ...
    const { driverType, driveLicence, setDriverData } = useDriverData();
    ...
    useEffect(() => {
        if(driveLicence) setPhoto(driveLicence);
    }, [driveLicence])
    ...
    const getProvince = async () => {
        ...
        const { data, error } = await API.post('/v2/driver/province', sender);
            ...
            <View style={{ flex: 1, backgroundColor: colors.white }}>
                <ScrollView >
                    <View style={styles.root} >
                        ...
                        <View style={{ gap: 33 }}>
                            <Image source={photo?.uri || (photo ? consts.s3Url + photo : images.license_card)} style={[styles.photo, { borderRadius: photo ? 23 : 0 }]} />
                            <Button onPress={startPhoto} >{lang({ id: photo ? 'retake_photo' : 'take_photo' })}</Button>
                        </View>
                        
                        {driverType === 2 && (
                            <View style={{ gap: 13 }}>
                                {gpsLoad && <Loading fixed color={colors.taseta} style={{ paddingBottom: 0, backgroundColor: 'rgba(255, 255, 255, 0.5)' }}/>}

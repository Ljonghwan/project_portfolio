...
export default function Page({ }) {
    ...
    const dataFunc = async (reset) => {
        ...
        const { data, error } = await API.post('/v2/info/reviewList', sender);

        setList(data || []);

        setTimeout(() => {

            initLoad && listRef?.current?.scrollToOffset({ offset: 0, animated: false });
    ...
    const renderItem = ({ item, index }) => {
        return (
            <Review item={item} style={{ marginBottom: 23 }} />
        )
    };
    ...
    const header = {
        ...
        title: lang({ id: 'reviews' }),
        filter: {
            icon: 'filter',
            list: [
                { idx: 1, title: lang({ id: 'most_recent' }) },
                { idx: 2, title: lang({ id: 'top_rated' }) },
            ],
            state: filter,
            onPress: filterPress
        }

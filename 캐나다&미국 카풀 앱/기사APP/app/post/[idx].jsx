...
export default function Page() {
    ...
    const [badges, setBadges] = useState({ status: false, chat: false })
    ...
    const renderScene = ({ route, jumpTo }) => {
        switch (route.key) {
            ...
            case 'chat':
                return <PostChat item={item} jumpTo={jumpTo} setBadges={setBadges} />;
            default:
                return null;
        }
    };
    ...
    const filterPress = (type) => {
        ...
        if (type === 3 && mbData?.idx === item?.userIdx) {

            openAlertFunc({
                label: lang({ id: 'delete_this_post' }),
                title: lang({ id: 'this_cannot_be' }),
    ...
    const deleteFunc = async () => {
        ...
        const { data, error } = await API.post('/v2/driver/post/delete', sender);
                    ...
                    <TabView
                        ...
                        commonOptions={{
                            ...
                            label: ({ route, labelText, focused, color }) => (
                                <Text style={[(focused ? styles.tabTextActive : styles.tabText), {  } ]}>{labelText ?? route.key}</Text>
                            ),
                            badge: ({ route }) => {
                                if (!badges?.[route.key]) return null;
                                return (

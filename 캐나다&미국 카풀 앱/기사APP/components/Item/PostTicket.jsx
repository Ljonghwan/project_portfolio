		...
		<Pressable style={[styles.container, style]} activeOpacity={1} onPress={onPressFunc}>
			<View style={{ gap: 15 }}>
				...
				<View style={[rootStyle.flex, { gap: 10, justifyContent: 'flex-start'  }]}>
					...
					<Text style={{ flex: 1, ...rootStyle.font(16, colors.main, fonts.medium) }}>{lang({ id: 'driver' })} - {lang({ id: item?.status === 4 ? 'end_ride' : item?.status === 3 ? 'on_ride' : 'pending' })}</Text>
				</View>
				...
				<View style={[rootStyle.flex, { gap: 10, justifyContent: 'flex-start' }]}>
					...
					<Text numberOfLines={1} style={{ flex: 1, ...rootStyle.font(18, colors.sub_1, fonts.medium) }}>
						{dayjs(`${item?.driveAt}`).format('MMM DD, YYYY, h:mm A')}
					</Text>
				</View>
				...
				<View style={[ rootStyle.flex, { gap: 7, justifyContent: 'flex-start' } ]}>
					...
					{item?.joins?.length > 0 && <Image source={images.dashed} style={rootStyle.dashed } />}

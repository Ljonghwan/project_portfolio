...
export default function Component({
    ...
    const openPop = () => {
        if (Platform.OS === 'ios') {
            openAlertFunc({
                component: <Calendar date={state} setDate={setState} />
            })
        } else {
            setView(!view);
        }
        ...
        <>
            <AnimatedTouchable style={[styles.root, animatedStyle, style]} onPress={openPop}>
                <View style={[rootStyle.flex, { flex: 1, justifyContent: 'flex-start', gap: 10 }]} >
                    <Image source={state ? images.calendar_black : images.calendar} style={rootStyle.default} transition={200} />
                    <Text style={[styles.title, { color: state ? colors.main : colors.sub_1 }]}>{state ? dayjs(state).format('MMMM/DD/YYYY') : placeholder}</Text>
                </View>
            ...
            {view && (
                <DateTimePicker
                    onChange={(event, date) => {
                        setState(dayjs(date).startOf('day'));
                        setView(false);
                    }}

...
export default function Component({
	...
	return (
		<View style={styles.root}>
			<View style={{ gap: 15 }}>
				<Text style={styles.title}>{lang({ id: 'decline_request' })}</Text>
				<Text style={{...rootStyle.font(16, colors.sub_1, fonts.medium), textAlign: 'center'}}>{lang({ id: 'are_you_sure_2' })}</Text>
				<View style={{ gap: 8 }}>
					<Select
						state={reason}
						setState={(v) => {
						...
						list={
							configOptions?.cancelDriver?.map((x, i) => {
				...
				<View style={[rootStyle.flex, { gap: 14 }]}>
					...
					<Button style={{ flex: 1 }} onPress={onPressFunc} disabled={!reason}>{lang({ id: 'decline' })}</Button>
				</View>
			</View>
		</View>
	);

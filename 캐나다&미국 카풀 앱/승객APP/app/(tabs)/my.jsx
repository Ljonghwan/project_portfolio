...
import { useUser, useAlert, useGps, useConfig, useLottie, useEtc } from '@/libs/store';
...
export default function Page() {
    ...
    const [links, setLinks] = useState([
        { title: 'emergency_contact', route: routes.emergency, level: [1] }, // 완료
        { title: 'history', route: routes.history, level: [1] }, // API 연결 필요

        { title: 'reviews', route: routes.reviews, level: [1], param: { idx: mbData?.idx } },  // 완료
        ...
        { title: 'blocked_list', route: routes.myBlock, level: [1] },  // 완료
        ...
        { title: 'payment_management', route: routes.notice, level: [2, 3] },
        { title: 'my_rides', route: routes.myRides, level: [2, 3] }, // 완료 (나의 운행정보 페이지 기사용)
        { title: 'my_vehicle', route: routes.notice, level: [2, 3] },
    ]);
    ...
    const dataFunc = async () => {
        ...
        setMsgView(
            ...
            (data?.idInfo?.status < 3 || data?.idInfo?.status === 4) ||
    ...
    const checkBadgeUpdate = async () => {
        ...
        if(mbData?.isBadgeUpdate ) {
            ...
            const { data, error } = await API.post('/v2/my/badgeUpdateCheck');
        }
    }
                ...
                <View style={{ gap: 25 }}>
                    ...
                    {msgView && (
                        <Animated.View entering={FadeInLeft} style={{ gap: 4 }}>
                            ...
                            {/* 신분증인증 신청 문구 */}
                            {idInfo?.status < 3 ? (
                                <Text style={styles.help}>{lang({ id: 'waiting_for_id' })}</Text>
                            ) : idInfo?.status === 4 && (
                                <Text style={[styles.help, { color: colors.text_popup }]}>{lang({ id: 'your_id_was_declined' })}</Text>
                            )}
                    ...
                    >
                        {links?.filter(item => !item?.level || item?.level?.includes(mbData?.passenger ? 1 : 0))?.map((x, i) => {
                            return (

...
import { formatCompact, numFormat, ToastMessage  } from '@/libs/utils';
...
export default function RevenueCard() {
    ...
    const dataFunc = async (reset) => {
        ...
        const { data, error } = await API.post('/v2/my/statReduction', sender);
        ...
        setTimeout(() => {
            ...
            setList(
                data?.labels?.map((v, i) => ({
                    ...
                    label: (range === 1 ? dayjs(v).format("MMM") : dayjs(v).format("D") ) || "??",
                    frontColor: '#7C9971F0',      // 기본색
                    gradientColor: '#BCD4B3',   // 그라데이션 하이라이트
                    topLabelComponent: () => (
                        <Text style={{ color: colors.taseta, fontSize: 6.5, marginBottom: 6, }}>{data?.values?.[i] || 0}</Text>
                    )
                }))
            )

            setTotal(data?.values?.reduce((acc, item) => acc + item, 0));
            // setTotal(999);
            // setList([]);
            setInitLoad(false);
            setLoad(false);
        ...
        <Layout header={header}>
            ...
            <View style={styles.root}>
                <View style={styles.titleBox}>
                    <Text style={styles.title}>{lang({ id: 'real_carbon_savings' })}</Text>
                    <Text style={styles.subTitle}>{lang({ id: 'track_your_carbon' })}</Text>
                </View>
                    ...
                    <View style={styles.header}>
                        <View>
                            ...
                            <Text style={styles.totalText}>
                                {formatCompact(total)} {lang({ id: 'kg' })}
                            </Text>
                        </View>
                ...
                <View style={{ alignItems: 'center', paddingHorizontal: rootStyle.side }}>
                    <Text style={{...rootStyle.font(18, colors.main, fonts.medium), textAlign: 'center'}}>
                        {lang({ id: 'youve_reduced' })} 
                        <Text style={{...rootStyle.font(18, colors.taseta, fonts.semiBold) }}> {numFormat(total)} {lang({ id: 'kg' })} </Text>
                        {lang({ id: range === 1 ? 'of_carbon_emissions_year' : 'of_carbon_emissions_month' })}
                    </Text>
                </View>

...
export default function Page() {
    ...
    const { id, type, typeText, hp, country, setSignData } = useSignData();
    ...
    const timeOut = () => {
        ...
        setError({...error, code: lang({ id: 'time_has_expired' })} );
        setTimer(false);
    }
    ...
    const submitFunc = async () => {
        ...
        const { data, error } = await API.post('/v2/passenger/user/checkAuth', sender);
        ...
        if(data?.result) {
            ...
            console.log('가입가능 O');
            if(type === 'email') {
                router.replace(routes.joinFormEmail);   
            } else {
            ...
            // 가입불가능
            console.log('가입불가능 X', data); 
            openAlertFunc({
                alertType: 'Sheet',
                labelIcon: data?.info?.type === 'google' ? images.google : data?.info?.type === 'apple' ? images.apple_black : images.email,
                label: `${data?.info?.email}`,
                title: `${lang({ id: 'already_signed_up' })}`,
                onCencleText: lang({ id: 'cancel' }),
                onPressText: lang({ id: 'go_log' }),
                onCencle: () => { },
                onPress: () => {
                    router.dismissAll();

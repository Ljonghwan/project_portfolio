...
export default function Page() {
    ...
    const loginFunc = async () => {
        ...
        const { data, error } = await API.post('/v2/passenger/user/login', sender);
    ...
    return (
        <Layout header={header} input={true} >
            <View
                style={styles.root}
            >
                <Text style={styles.title}>{lang({ id: 'hello' })}</Text>
                ...
                <View style={{ gap: 11, marginBottom: 14 }}>
                    ...
                    <Input
                        ...
                        setState={setpw}
                        name={'pw'}
                        placeholder={lang({ id: 'password_1' })}
                        password
                        returnKeyType={"done"}
                ...
                <View style={{ gap: 17 }}>
                    <Button onPress={loginFunc} load={load}>{lang({ id: 'log' })}</Button>
                    <TouchableOpacity style={{ alignSelf: 'flex-end' }} hitSlop={10} onPress={findStart}>
                        <Text style={styles.help}>{lang({ id: 'find_passwords' })}</Text>
                    </TouchableOpacity>
                </View>
            ...
            <View style={styles.bottom}>
                <Text style={[styles.help, { textAlign: 'center' }]}>{lang({ id: 'arent_you_member' })}</Text>
                <Button 
                    type={2} 
                    onPress={joinStart}
                >

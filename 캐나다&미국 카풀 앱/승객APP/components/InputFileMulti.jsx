...
export default function Component({ style, styleImage, styleCamera, photo=[], setPhoto, label, label2 }) {
    ...
    const photoFunc = () => {
        ...
        if(photo?.length >= 3) {
            ToastMessage(lang({ id: 'max_images_limit' })?.replace("{number}", "3"), { type: 'error'});
            return;
        }
        openPhotoFunc({
            setPhoto: (v) => setPhoto((prev) => [...prev, ...v]),
            selectionLimit: 3 - photo?.length
        })
    ...
    return (
        <View style={[styles.profileBox, style]} >
            {photo?.map((x, i) => {
                return (
                    <TouchableOpacity key={i} style={styles.label} onPress={() => imageViewer({ index: i, list: photo?.map(xx => xx?.uri) })}>
                        <Image 
                            source={x?.uri} 
                            style={styles.labelImage} 
                        />
                        <TouchableOpacity style={styles.profileCamera} onPress={() => setPhoto((prev) => prev?.filter((xx, ii) => ii !== i))}>
                            <Image 

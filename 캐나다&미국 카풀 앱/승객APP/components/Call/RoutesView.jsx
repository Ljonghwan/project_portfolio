...
export default function Page() {
    ...
    return (
        <View style={styles.bottom}>
            ...
            <View style={{ gap: 28, paddingHorizontal: rootStyle.side }}>
                <Text style={{textAlign: 'center', ...rootStyle.font(20, colors.main, fonts.semiBold)}}>{lang({ id: 'check_your_route_details' })}</Text>
                ...
                <View style={{ gap: 25 }} onLayout={onLayout}>
                    ...
                    <View style={[rootStyle.flex, { gap: 16, justifyContent: 'flex-start' }]}>
                        ...
                        <Text numberOfLines={2} style={{ flex: 1, ...rootStyle.font(18, colors.main, fonts.medium)}}>{lang({ id: 'departure'})} : {start?.name}</Text>
                    </View>
                    {way?.map((x, i) => {
                        return (
                            <View key={i} style={[rootStyle.flex, { gap: 16, justifyContent: 'flex-start' }]}>
                                ...
                                <Text numberOfLines={2} style={{ flex: 1, ...rootStyle.font(18, colors.main, fonts.medium)}}>{lang({ id: 'stop'})} : {x?.name}</Text>
                            </View>
                        )
                    })}
                    <View style={[rootStyle.flex, { gap: 16, justifyContent: 'flex-start' }]}>
                        ...
                        <Text numberOfLines={2} style={{ flex: 1, ...rootStyle.font(18, colors.main, fonts.medium)}}>{lang({ id: 'destination'})} : {end?.name}</Text>
                    </View>
                </View>
            </View>
        </View>

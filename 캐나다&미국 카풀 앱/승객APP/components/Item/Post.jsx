...
export default function Component({ item, style, way=false }) {
	...
	const onFavFunc = async () => {
		
		const sender = {
			postIdx: item?.idx
		}

		const { data, error } = await API.post('/v2/passenger/post/favInsert', sender);
	...
	return (
		<Pressable style={[styles.container, style]} activeOpacity={1} onPress={onPress}>
			<View style={[styles.top]}>
				<View style={{ flex: 1 }}>
					<View style={[rootStyle.flex, { justifyContent: 'space-between', marginBottom: 18 }]}>
						<TouchableOpacity onPress={onFavFunc}>
							<Image source={fav ? images.bookmark_on : images.bookmark_off} style={rootStyle.default} />
							{/* <Image source={images.bookmark_on} style={rootStyle.default} /> */}
						</TouchableOpacity>
					...
					<View style={[rootStyle.flex, { gap: 10, justifyContent: 'flex-start', marginBottom: 10 }]}>
					...
					{way ? (
						<RoutesView style={{ gap: 15, marginBottom: 21 }} way={item?.itinerary} />
					) : (
				...
				<View style={[rootStyle.flex, { justifyContent: 'space-between' }]}>
					...
					<View style={[{ alignItems: 'flex-end',  gap: 2 }]}>
						<Text numberOfLines={1} style={{ ...rootStyle.font(16, colors.main, fonts.semiBold) }}>
							{numDoler( sumPay({ itinerary: item?.itinerary, startIndex: item?.startIndex || 0, endIndex: item?.endIndex || item?.itinerary?.length - 1, rate: item?.feeRate }) )}
						</Text>
						{status ? 
							<Text style={{ ...rootStyle.font(14, colors.text_popup, fonts.medium) }}>{item?.seats - item?.joinCount} {lang({ id: 'seat_left' })}</Text>
						: 
							<Text style={{ ...rootStyle.font(14, colors.text_popup, fonts.medium) }}>{lang({ id: 'end_of_recruitment' })}</Text>
						}
					</View>
				</View>
			</View>

	...
	return (
		<TouchableOpacity style={[styles.container, style]} activeOpacity={1} onPress={onPressFunc}>
			...
			<View style={[styles.top]}>
				...
				<View style={{ gap: 12 }} >
					...
					<View style={[rootStyle.flex, { justifyContent: 'flex-start', gap: 6 }]}>
						<Image source={consts.s3Url + mbData?.profile} style={{ width: 30, height: 30, borderRadius: 1000, backgroundColor: colors.placeholder }} />
						<View style={[rootStyle.flex, { flex: 1, gap: 4 }]}>
							<View style={[rootStyle.flex, { flex: 1, justifyContent: 'flex-start', gap: 4 }]}>
								...
								<Text numberOfLines={1} style={{ flexShrink: 1, ...rootStyle.font(16, colors.main, fonts.semiBold) }}>{mbData?.firstName} {mbData?.lastName}</Text>
							</View>
			...
			<View style={styles.bottom}>
				...
				<View style={[rootStyle.flex, { justifyContent: 'flex-end' }]}>
					{item?.joins?.length > 0 ? (
						<Text style={{ ...rootStyle.font(18, colors.text_popup, fonts.medium) }}>{item?.joins?.length} {lang({ id: 'application' })}</Text>
					) : (

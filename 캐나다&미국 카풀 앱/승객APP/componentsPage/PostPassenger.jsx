...
export default function Page({ item, reload, setReload, jumpTo, dataFunc }) {
    ...
    const onFinishFunc = async (v) => {
        ...
        const { data, error } = await API.post('/v2/passenger/reqpost/finish', sender);
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            {(mbData?.idx === item?.creator?.idx) && (
                <Button type={item?.status === 1 ? 2 : 1} bottom onPress={onFinishFunc}>{lang({ id: item?.status === 1 ? 'matched' : 'searching' })}</Button>
            )}

...
export default function Page({ item, reload, setReload, jumpTo, dataFunc }) {
    ...
    useEffect(() => {
        ...
        if (start && end) {
            let pay = sumPay({ itinerary: item?.itinerary, startIndex: item?.itinerary?.findIndex(x => x?.idx === start?.idx), endIndex: item?.itinerary?.findIndex(x => x?.idx === end?.idx), rate: item?.feeRate })
            setPaySum(pay);
        }
    ...
    const pathChangeFunc = () => {
        openAlertFunc({
            component: <PathChange list={item?.itinerary} start={start} setStart={setStart} end={end} setEnd={setEnd} />
        })
    }
    ...
    const chatFunc = async () => {
        ...
        if (!chatIdx) {
            ...
            const { data, error } = await API.post('/v2/chat/createChat', sender);
        ...
        <View style={{ flex: 1 }}>
            ...
            >
                    ...
                    <View style={[rootStyle.flex, { justifyContent: 'flex-start', gap: 10 }]}>
                        ...
                        <View style={[{ flex: 1, gap: 2 }]}>
                            <View style={[rootStyle.flex, { justifyContent: 'flex-start', gap: 6 }]}>
                                {/* <Image source={consts.s3Url + treeBadge?.imgPath} style={{ width: 26, height: 26, borderRadius: 1000 }} /> */}
                    ...
                    {item?.itinerary?.map((x, i) => {
                        return (
                            <View key={i} style={[rootStyle.flex, { gap: 16, justifyContent: 'flex-start' }]}>
                                ...
                                <View style={{ flex: 1, gap: 4, opacity: (x?.idx === start?.idx || x?.idx === end?.idx) ? 1 : 0.2 }}>
                                ...
                                {x?.idx === start?.idx && (
                                    <Tag msg={lang({ id: 'boarding' })} />
                                )}
                                {x?.idx === end?.idx && (
                                    <Tag msg={lang({ id: 'stopover' })} />
                                )}
                ...
                <View style={{ gap: 10 }}>
                    <View style={[rootStyle.flex, { gap: 10, justifyContent: 'flex-start', }]}>
                        ...
                        {/*                         
                        {item?.isJoin ? (
                            myJoinInfo?.status === 1 ? <Tag msg={lang({ id: 'pending_approval' })} />
                                : myJoinInfo?.status === 2 ? <Tag msg={lang({ id: 'approval' })} />
                                    : <Tag type={3} msg={lang({ id: 'cancel' })} />
                        ) : (
                            status ? <Tag msg={lang({ id: 'recruitment' })} /> : <Tag type={3} msg={lang({ id: 'finished' })} />
                        )} */}
                ...
                <View style={{ gap: 20, marginTop: 30 }}>
                    ...
                    <Button onPress={chatFunc}>{lang({ id: 'send_a_message_to_the_driver' })}</Button>
                </View>
                ...
                <TouchableOpacity style={styles.link} onPress={() => { jumpTo('status') }}>
                    <View style={[rootStyle.flex, { flex: 1, gap: 3, justifyContent: 'flex-start', }]}>
                        ...
                        <Text numberOfLines={1} style={{ flex: 1, ...rootStyle.font(18, colors.main, fonts.medium) }}>{confirmCount} {lang({ id: 'confirmed' })} | {item?.seats - confirmCount} {lang({ id: 'seat_left' })}</Text>
                    </View>
                    ...
                    <View style={[rootStyle.flex, { justifyContent: 'space-between', paddingBottom: insets?.bottom + 20, paddingHorizontal: rootStyle.side }]}>
                        <View style={{ gap: 5, alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            {item?.itinerary?.length > 2 ? (
                                ...
                                <View style={[styles.pointTagDisabled, {}]} >
                                    <Text style={{ ...rootStyle.font(14, colors.sub_2, fonts.medium) }}>{lang({ id: 'path_change' })}</Text>
                                </View>
                            )}
            ...
            {(item?.isJoin && item?.status < 4) && (
                item?.status === 3 ? (
                    ...
                    <Button bottom disabled type={2}>{lang({ id: 'trip_pending_start' })}</Button>
                )
            )}

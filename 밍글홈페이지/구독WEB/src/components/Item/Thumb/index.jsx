...
export default function Component({
    ...
    return (
        <div id={`portpolio-${item?.idx}`} className={'portpolio_one'} style={{ ...style }} data-scroll-section>
            ...
            >
                <div className='dim'>
                    <img className={'bg_img'} src={item?.thumb ? (consts.s3Url + item?.thumb) : "/og.png"} data-scroll data-scroll-speed="1.5" data-scroll-offset={ size?.width > 768 ? "25%, 15%" : "0"} data-scroll-call="onEnter" data-scroll-repeat data-bg={`${item?.bg}`} />

                    {item?.thumbVideo ? (
                        <video
                            className={'bg_hover'}
                            src={consts.s3Url + (item?.thumbVideo)}
                            muted
                            playsInline
                            loop
                            autoPlay
                        ...
                        <img className={'bg_hover'} src={item?.thumbBg ? (consts.s3Url + item?.thumbBg) : "og.png"} />
                    )}
                </div>

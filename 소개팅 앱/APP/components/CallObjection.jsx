...
export default function Component({
    ...
    const inputFunc = (item) => {
        ...
        try {
            ...
            openAlertFunc({
                component: <CallObjectionTextInput item={item} desc={desc} onSubmit={onSubmit}/>,
                input: 200
            })   
                    ...
                    <View style={styles.root}>
                        ...
                        >
                            <View style={ styles.titleBox }>
                                <Text style={styles.title}>소명 유형</Text>
                                <Text style={styles.subTitle}>소명 유형을 선택해 주세요.</Text>
                            </View>
                            <View>
                                {configOptions?.objectionOptions?.map((x, i) => {
                                    return (
                                        <TouchableOpacity key={x?.idx} style={[styles.list, value === x?.idx && styles.listActive]} onPress={() => {
                                            if(x?.isDesc) inputFunc(x);
                                            else onSubmit({ optionIdx: x?.idx })
                                        }}>
                                            <Text style={[styles.listText, value === x?.idx && styles.listTextActive]}>{x?.title}</Text>
                                        </TouchableOpacity>
                                    )
                                })}
                            </View>

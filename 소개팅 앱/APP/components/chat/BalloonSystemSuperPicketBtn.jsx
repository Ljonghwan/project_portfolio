    ...
    return (
        <AnimatedBackground bg={chatTheme?.systemBackgroundColor} style={[styles.root, { borderColor: chatTheme?.primary }]}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center', marginBottom: -4 }}>
                <Image source={chatImages.chat_ticket} style={rootStyle.default32} transition={200} tintColor={chatTheme?.primary} />
                <Image source={chatImages.chat_ticket_star} style={[{ width: 11.7, height: 11.2, position: 'absolute' }]} />
            </View>

            <Text style={{ ...rootStyle.font(16, colors.black, fonts.medium), lineHeight: 20, textAlign: 'center' }}>{item?.message}</Text>

            <Image source={user?.profile ? consts.s3Url + user?.profile : images.profile} style={[{ width: 48, height: 48, borderRadius: 1000, backgroundColor: colors.greyD9 }]} transition={200}/>

            <Text style={{ ...rootStyle.font(14, colors.dark, fonts.regular), lineHeight: 20, textAlign: 'center' }}>{item?.data?.subMessage}</Text>

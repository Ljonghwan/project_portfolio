...
export default function Component({ item, time=null, product }) {
    ...
    const handleSubmit = async () => {
        ...
        router.navigate({
            ...
            params: {
                ...
                productIdx: saleProduct?.idx
            }
        })
    }

    const normalProduct = product?.find(x => x?.type === 2);
    const saleProduct = product?.find(x => x?.type === 3);
            ...
            >
                    ...
                    <View style={{ gap: 8 }}>
                        <View style={{ gap: 12 }}>
                            ...
                            <Text style={styles.title}>
                                {`12시간 이내에 리매치하시면\n`}
                                <Text style={[styles.title, { fontSize: 22, fontFamily: fonts.bold }]}>{numFormat(66000)}원</Text>
                                {`으로\n재소개받으실 수 있어요.`}
                            </Text>
                        </View>

                        <View style={styles.timer}>
                            <Image source={images.clock_main} style={rootStyle.default20} />
                ...
                <View style={styles.buttonBox}>
                    ...
                    <TouchableOpacity style={[styles.button, { backgroundColor: colors.main }]} onPress={handleSubmit}>
                        <Text style={styles.buttonOkText}>할인 결제 이용하기</Text>
                        ...
                        <View>
                            <Text style={[styles.buttonOkText, { textDecorationLine: 'line-through' }]}>{numFormat(normalProduct?.price)}원</Text>
                        </View>
                        
                        <Text style={[styles.buttonOkText, { fontSize: 18 }]}>{numFormat(saleProduct?.price)}원</Text>
                    </TouchableOpacity>
                </View>
            </ImageBackground >
        </TouchableWithoutFeedback>

...
import Feed3 from '@/components/list/Feed3';
...
export default function Page({ }) {
    ...
    const dataFunc = async (reset = false) => {
        ...
        const { data, error } = await API.post('/v1/feed/list', sender);

        setNextToken(data?.nextToken || null);

        const fetchData = data?.list || [];
        // const fetchData = [...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list, ...data?.list,];
    ...
    const renderItem = ({ item, index }) => {
        return (
            <Feed3 item={item} index={index} viewables={viewables} />
        );
    };
    ...
    return (
        <Layout header={header} >
            ...
            <Order value={order} view={orderView} handleClose={() => setOrderView(false)} onSubmit={orderFunc} />
        </Layout>
    );
}

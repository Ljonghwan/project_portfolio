...
export default function Component({
    ...
    const renderItem = ({ item, index }) => {
        return (
            <ListItem style={{ maxWidth: '100%' }} delay={index * 20} active={value === item} title={item} onPress={() => setValue({ value: item, name: name })} />
        );
    };
    ...
    return (
        <View style={styles.root}>
            ...
            <Animated.View entering={FadeInLeft}>
                <Text style={styles.title}>{target ? '원하는 이성의 흡연 여부는 어떤가요?' : '회원님은 흡연을 하시나요?'}</Text>
            </Animated.View>

            <View style={[rootStyle.flex, { flex: 1 }]}>
                <FlatList
                    indicatorStyle={'black'}
                    data={configOptions?.smokeOptions}

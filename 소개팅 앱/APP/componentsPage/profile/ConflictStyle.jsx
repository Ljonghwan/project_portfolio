    ...
    return (
        ...
        >
            <Animated.View entering={FadeIn}>
                <Text style={styles.title}>갈등이 생겼을 때 나는 이런 스타일이에요!</Text>
            </Animated.View>
            
            <View style={[{ flex: 1 }]}>
                <Animated.View key={value || 'default'} entering={FadeIn} style={{ marginBottom: 0 }}>
                    <Image source={consts.s3Url + (configOptions?.conflictStyleOptions?.find(v => v?.title === value)?.icon || configOptions?.conflictStyleOptions?.[0]?.icon)} style={styles.styleImage} />
                </Animated.View>
                <FlatList
                    data={configOptions?.conflictStyleOptions}

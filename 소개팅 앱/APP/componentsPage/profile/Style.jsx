    ...
    return (
        ...
        >
            <Animated.View entering={FadeIn}>
                <Text style={styles.title}>나의 연애 스타일은?</Text>
            </Animated.View>
            <View style={[{ flex: 1 }]}>
                <Animated.View key={value || 'default'} entering={FadeIn} style={{ marginBottom: 0 }}>
                    <Image source={consts.s3Url + (configOptions?.styleOptions?.find(v => v?.title === value)?.icon || configOptions?.styleOptions?.[0]?.icon)} style={styles.styleImage} />
                </Animated.View>
                <FlatList
                    data={configOptions?.styleOptions}

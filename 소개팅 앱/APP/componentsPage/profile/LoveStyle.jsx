    ...
    return (
        ...
        >
            <Animated.View entering={FadeIn}>
                <Text style={styles.title}>사랑앞에 나는 이런 스타일이에요!</Text>
            </Animated.View>
            
            <View style={[{ flex: 1 }]}>
                <Animated.View key={value || 'default'} entering={FadeIn} style={{ marginBottom: 0 }}>
                    <Image source={consts.s3Url + (configOptions?.loveStyleOptions?.find(v => v?.title === value)?.icon || configOptions?.loveStyleOptions?.[0]?.icon)} style={styles.styleImage} />
                </Animated.View>
                <FlatList
                    data={configOptions?.loveStyleOptions}

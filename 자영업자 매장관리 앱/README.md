...
- **계약서 저장:** 표준 근로계약서에 대상 정보를 기입후 서명하여 저장

### 9. 제품 원가 관리 (Work09)
- **상품별 원가:** 판매 상품의 상세 원가 분석
- **수익성 분석:** 상품별 마진율 비교
- **수익성 변동감지:** 매입 관리(Work01)와 원가 계산기(Work02)의 데이터를 사용하여, 일자별로 변동률 감지
...
5. **전자 서명 및 PDF 생성:** Canvas 기반 서명 캡처 → pdf-lib를 이용한 계약서 PDF 생성 → S3 저장 → 앱 내 뷰어 및 다운로드

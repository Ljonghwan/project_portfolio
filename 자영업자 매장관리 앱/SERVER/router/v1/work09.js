...
router.post('/list', async (req, res) => {
    ...
    let result = await models.ITEM_COST_TB.scope([
        'active',
        'desc',
        'info',
    ...
    let result2 = await models.ITEM_COST_GROUP_TB.scope([
        'active',
        'desc',
        'list',
        { method: ['my', store?.idx || 0] },
    ...
    let list = await Promise.all([...result, ...result2]?.map(x => getItemCostGroupInfo(x)));
...
router.post('/get', async (req, res) => {
    ...
    let { idx = 0, type, store, user, nowDt, ip } = req.body;
    ...
    let row = null;
    if(type === '일반') {
        row = await models.ITEM_COST_TB.scope([
            'active',
            'desc',
            'info',
            { method: ['my', store?.idx || 0] },
        ...
        row = await models.ITEM_COST_GROUP_TB.scope([
            'active',
            'desc',
            'list',
            { method: ['my', store?.idx || 0] },
        ...
        row = await getItemCostGroupInfo(row);
    }
 ...
 *              "type": "object",
 *              "properties": {
 *                "idx": { "type": "number | null", "example":1, "description": "제품 원가 고유번호" },
 *                "company": { "type": "string", "example": "거래처명", "description": "거래처명" },
 *                "title": { "type": "string", "example": "품목명", "description": "품목명" },
 *                "unit": { "type": "string", "example": "g", "description": "단위" },
 *                "volume": { "type": "number", "example": 100, "description": "용량" },
 *                "yield": { "type": "number", "example": 100, "description": "수율" },
 *              },
 *            }
 *          }
 *        }
...
router.post('/update', async (req, res) => {
    ...
    let { idx, company, title, unit, volume, yield, store, user, nowDt, ip } = req.body;

    // 필수 파라미터 검증
    if (!company || !title || !unit || !volume || !yield) {
        sendError(req, res, "params");
        return;
    }
    ...
    try {
        ...
        if (idx) {
            const row = await models.ITEM_COST_TB.findOne({
                where: {
                    idx: idx,
                    user_idx: user?.idx
                }
            ...
            let createData = await models.ITEM_COST_TB.create(ob)
            idx = createData?.idx;
        }

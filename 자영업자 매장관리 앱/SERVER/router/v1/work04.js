...
const { send, sendError, deleteFile, saveImages } = require('../../service/utils.js');
const { encrypt, decrypt } = require('../../service/crypto.js');
const { generateStaffContract } = require('../../pdf/service.js');
...
router.post('/list', async (req, res) => {
    ...
    let result = await models.STAFF_TB.scope([
        'active',
        'desc',
...
router.post('/get', async (req, res) => {
    ...
    let row = await models.STAFF_TB.scope([
        'active',
        'desc',
        { method: ['my', store?.idx || 0] },
    ]).findOne({
 ...
 *                "rrn_last": { "type": "string", "example": "123456", "description": "주민번호 뒷자리" },
 *                "hp": { "type": "string", "example": "01012345678", "description": "휴대폰 번호" },
 *                "sdate": { "type": "string", "example": "2025-01-01", "description": "근무 시작일" },
 *                "edate": { "type": "string | null", "example": "2025-01-01", "description": "근무 종료일" },
 *                "type": { "type": "number", "example": 1, "description": "고용 형태" },
 ...
 *                "pay_type": { "type": "number", "example": 1, "description": "급여 지급 종류" },
 *                "pay": { "type": "number", "example": 10000, "description": "시급 또는 월급" },
 *              },
 *            }
 *          }
 *        }
...
router.post('/update', async (req, res) => {
    ...
    let { idx, name, rrn_first, rrn_last, hp, sdate, edate, type, work_type, work_day, work_stime, work_etime, pay_type, pay, store, user, nowDt, ip } = req.body;

    if(!name || !rrn_first || !rrn_last || !hp || !sdate || !type || work_day?.length < 1 || !work_stime || !work_etime || !pay_type || !pay) {
    ...
    try {
        let ob = {
            ...
            edate: edate ? parseInt((edate + "")?.replace(/-/g, '')) : null,
            type: type,
            work_type: work_type,
            work_day: work_day,
        ...
        if (idx) {
            const row = await models.STAFF_TB.findOne({
                where: {
                    idx: idx,
                    user_idx: user?.idx
                }
            ...
            ob.rrn_first = encrypt(rrn_first);
            ob.rrn_last = encrypt(rrn_last);

            let createData = await models.STAFF_TB.create(ob)
            idx = createData?.idx;
        }
    ...
    try {
        ...
        const result = await generateStaffContract({ staff: row, store: store });
...
router.post('/leave', async (req, res) => {
    ...
    let { idx=0, edate, store, user, nowDt, ip } = req.body;

    try {
        let row = await models.STAFF_TB.findOne({
            where: {
                idx: idx,
                user_idx: user?.idx
            }
        ...
        if (!row) throw new Error("auth_fail");
        if(!edate) throw new Error("params");

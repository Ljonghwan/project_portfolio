	...
	return (
		<View style={styles.root}>
			...
			<Animated.View style={{ gap: 16 }}>
				<View style={{ gap: 8 }}>
					<Text style={{ ...rootStyle.font(20, colors.black, fonts.bold) }}>내 매장</Text>
				</View>
				<View>
					{mbData?.store?.map((x, i) => {
						return (
							<TouchableOpacity key={i} activeOpacity={0.7} style={[rootStyle.flex, { justifyContent: 'space-between', paddingVertical: 12 }]} onPress={() => {
								if(filter === 'cardsales' && x?.cardsales_grp_id) return;
								
								setSelected(x?.idx)
							}}>
								<Text style={{ ...rootStyle.font(15, colors.black) }}>{x?.title}</Text>
								{filter === 'cardsales' && x?.cardsales_grp_id ? (
									<TouchableOpacity 
										style={[rootStyle.flex, { gap: 2 }]} 
										activeOpacity={0.7} 
										onPress={() => {
											onClear(x);
										}}
									>
										<Text style={{ ...rootStyle.font(12, colors.primary) }}>연동 해제</Text>
										<Image source={images.link2} style={rootStyle.default16} />
									</TouchableOpacity>
								) : (
			...
			<View style={[rootStyle.flex, { gap: 4 }]}>
				<Button disabled={disabled} load={load} style={{ flex: 1 }} onPress={onSubmitAlert}>선택</Button>
			</View>
		</View>
	);
}

...
export default function Page({ cate, page }) {
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <Pressable style={{ gap: 14, paddingVertical: 12, paddingHorizontal: 4, borderBottomColor: colors.f8f8f9, borderBottomWidth: 1 }} onPress={() => {
                router.push({
                ...
                <View style={{ flex: 1, gap: 10 }}>
                    <Text style={{...rootStyle.font(15, colors.text2B2B2B, fonts.semiBold), lineHeight: 22 }} numberOfLines={2}>{item?.title}</Text>
                    <Text style={{...rootStyle.font(12, colors.text757575, fonts.medium) }}>{dayjs(item?.createAt).format('YYYY.MM.DD')}</Text>
                </View>
            </Pressable>
        );
    };
        ...
        <Layout header={header} >
            <View style={{ flex: 1 }}>
                ...
                <FlashList
                    ...
                    ListEmptyComponent={
                        <Empty msg={'공지사항이 없습니다.'} style={{ paddingBottom: 0 }} />
                    }
                />

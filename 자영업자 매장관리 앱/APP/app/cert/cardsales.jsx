...
export default function Page() {
    ...
    useEffect(() => {
        const isValid = userId?.length >= 6 && password?.length >= 8;
        setDisabled(!isValid);
    }, [userId, password])
    ...
    const submitFunc = async () => {
        ...
        router.replace({
            pathname: routes.certCardSalesWait,
            params: {
                idx: idx,
                userId: userId,
                password: password
        ...
        <Layout header={header} statusBar={'dark'} >
            ...
            >
                ...
                <View
                    style={styles.root}
                >
                    <Text style={[styles.title, { marginBottom: 40 }]}>{`서비스에 연동하고,\n매출을 간편하게 확인해보세요!`}</Text>

                    <View style={{}}>
                        {/* 아이디 */}
                        <View style={styles.fieldContainer}>
                            <Text style={styles.label}>여신금융협회 아이디</Text>
                            <TextInput
                                // autoFocus
                                iref={idRef}
                                value={userId}
                                onChangeText={setUserId}
                                placeholder="여신금융협회 아이디를 입력해주세요."
                                maxLength={20}
                                returnKeyType="next"
                                blurOnSubmit={false}
                        ...
                        {/* 비밀번호 */}
                        <View style={styles.fieldContainer}>
                            <Text style={styles.label}>여신금융협회 비밀번호</Text>
                            <TextInput
                                iref={passwordRef}
                                value={password}
                                onChangeText={setPassword}
                                placeholder="여신금융협회 비밀번호를 입력해주세요."
                                secureTextEntry
                                returnKeyType="done"
            ...
            <Button bottom style={{ position: 'absolute', bottom: 0 }} disabled={disabled} onPress={openSheet} >연동하기</Button>
            ...
            >
                <View style={styles.sheet}>
                    <View style={{ gap: 19 }}>
                        <Text style={styles.sheetTitle}>(필수) 서비스 연동 이용 동의</Text>

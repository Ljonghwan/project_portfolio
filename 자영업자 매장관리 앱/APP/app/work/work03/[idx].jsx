...
export default function Page({ }) {
    ...
    const onDelete = async () => {
        ...
        const { data, error } = await API.post('/v1/work03/delete', sender);
    ...
    return (
        <Layout header={header} >
            ...
            >
                <View style={styles.box}>
                    ...
                    <View style={[ rootStyle.flex, { gap: 10, justifyContent: 'space-between' }]}>
                        ...
                        <Text style={{...rootStyle.font(width <= 320 ? 14 : 16, colors.header, fonts.semiBold), flexShrink: 1 }} numberOfLines={1}>{item?.rrn_first} - {item?.rrn_last ? item?.rrn_last?.at(0) + '******' : ''}</Text>
                    </View>
                    <View style={[ rootStyle.flex, { gap: 10, justifyContent: 'space-between' }]}>
                ...
                <View style={{ gap: 18 }}>  
                    ...
                    <View style={styles.box}>
                        ...
                        <View style={[ rootStyle.flex, { gap: 10, justifyContent: 'space-between' }]}>
                            <Text style={{...rootStyle.font(14, colors.text757575, fonts.medium)}}>시급</Text>
                            <Text style={{...rootStyle.font(16, colors.header, fonts.semiBold), flexShrink: 1 }} numberOfLines={1}>{numFormat(item?.pay) + ' 원'}</Text>
                        </View>
                        <View style={[ rootStyle.flex, { gap: 10, justifyContent: 'space-between' }]}>
                            <Text style={{...rootStyle.font(14, colors.text757575, fonts.medium)}}>일당</Text>
                            <Text style={{...rootStyle.font(16, colors.header, fonts.semiBold), flexShrink: 1 }} numberOfLines={1}>{numFormat(item?.pay_calc) + ' 원'}</Text>
                        </View>
                        <View style={[ rootStyle.flex, { gap: 10, justifyContent: 'space-between' }]}>
                            <Text style={{...rootStyle.font(14, colors.text757575, fonts.medium)}}>비고</Text>
                            <Text style={{...rootStyle.font(16, colors.header, fonts.semiBold), flexShrink: 1 }}>{item?.memo || '-'}</Text>
                        </View>
                    </View>
                </View>
            ...
            <BottomMultiButton>
                {(item?.idx && item?.status !== 9) && <Button type={'delete'} style={{ flex: 1, }} onPress={onDeleteAlert} >삭제</Button>}
                <Button load={load} style={{ flex: 1 }} onPress={() => {
                    if(item?.cert === 1) {
                    router.push({
                        pathname: routes.일용노무등록,
                            params: { idx: item?.idx }
                        });
                    } else {
                        router.push({

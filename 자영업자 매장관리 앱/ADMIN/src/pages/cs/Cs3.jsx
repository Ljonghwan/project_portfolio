...
import FaqInfoBox from "@/components/Box/FaqInfoBox";
...
export default function Cs4() {
    ...
    async function loadData() {
        ...
        setLoad(true);
        const { data, error } = await API.post('/admin/faq/list', sender);
    ...
    return <div className="box">
        ...
        <div className="page_contain">
                ...
                <DataTable
                    ...
                    body={(item, index) => {
                        return <tr key={"cs4-ti" + index} className="hand" onClick={() => {
                            ...
                            <td>{item?.idx}</td>
                            <td>{configOptions?.faqOptions?.find(x => x?.idx === item?.cate)?.title}</td>
                            <td>{item?.title}</td>
            ...
            {show &&
                <FaqInfoBox detail={detail} dataList={dataList} onUpdate={loadData} closeFunc={() => {
                    setShow(false);
                }} />
            }
        </div >

...
export default function Component({
    ...
    useEffect(() => {
        ...
        if (list?.length > 0) {
            setViewlist( list?.filter(filterFunc) )
        } else {
            setViewlist([]);
        }
    ...
    const filterFunc = (item, index) => {
        return (
            ...
            (sdate ? dayjs(item.deleteAt).isSameOrAfter(dayjs(sdate), 'day') : true) &&
            (edate ? dayjs(item.deleteAt).isSameOrBefore(dayjs(edate), 'day') : true) &&
            (debouncedStx ?
                (
    ...
    const recoverFunc = async (idx) => {
        ...
        const { data, error } = await API.post('/admin/portpolio/recover', sender, { id });
    ...
    const deleteAlert = async (idx) => {
        ...
        openPopup({
            title: '삭제하기',
            message: "포트폴리오를 영구 삭제 하시겠습니까?",
            warning: `이 동작은 실행 취소할 수 없습니다.`,
            buttonCencle: "취소",
    ...
    return (
        <>
                                ...
                                <>
                                    ...
                                    <td>
                                        <div className='flex' style={{ justifyContent: 'center', gap: 8 }}>
                                            <button className='td_btn type2' onClick={(e) => {
                                                e.stopPropagation();
                                                recoverFunc(item?.idx);
                                            }}>복구</button>
                                            <button className='td_btn type3' onClick={(e) => {
                                                e.stopPropagation();
            ...
            {show &&
                <PortpolioInfoBox detail={detail} setDetail={setDetail} closeFunc={closeFunc} recoverFunc={recoverFunc} deleteAlert={deleteAlert} />
            }
        </>
    )
}

...
export default function Page() {
    ...
    const { allData, loading } = useAllData({
        ...
        params: { types: ['earn', 'refund'] },
        deps: ['point_earn'],
    });

    // reason 종류는 서버 코드에서 다양하게 저장되므로 allData에서 동적으로 추출
    ...
    const handleExcelDownload = () => {
        ...
        const headers = ['NO', '획득일시', '아이디', '이름', '닉네임', '획득포인트', '획득내역'];
        ...
        a.href = url; a.download = `획득내역_${dayjs().format('YYYYMMDD')}.csv`;
        a.click(); URL.revokeObjectURL(url);
    };

    const columns = [
        noColumn({ totalCount, page, limit: LIMIT }),
        dateColumn('획득일시', 'createdAt'),
        ...
        numberColumn('획득포인트', 'amount', { suffix: 'P' }),
        { title: '획득내역', dataIndex: 'reason', width: 180, render: (v) => v || '-' },
    ];

    return (
        <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                <Title level={4} style={{ margin: 0 }}>획득 내역</Title>
            ...
            <Card style={{ marginBottom: 24 }}>
                <Form layout="vertical">
                    <Row gutter={16} align="bottom">
                        <Col span={8}>
                            <Form.Item label="획득일시" style={{ marginBottom: 12 }}>
                                <RangePicker
                        ...
                        <Col span={4}>
                            <Form.Item label="획득내역" style={{ marginBottom: 12 }}>
                                <Select
